...
class AMRMBart50Tokenizer(MBart50Tokenizer):
    ...
    def init_amr_vocabulary(self, pred_min=5):
        for tok in [self.bos_token, self.eos_token, self.pad_token, '<mask>', '<unk>']:
            ntok = self.INIT + tok
            i = self.fairseq_tokens_to_ids[tok]
            del self.fairseq_tokens_to_ids[tok]
            self.fairseq_tokens_to_ids[ntok] = i
        ...
        self.old_enc_size = enc_size = self.vocab_size
        for t in tokens:
    ...
    def vocab_size(self) -> int:
        return len(self.sp_model) + len(self.fairseq_tokens_to_ids) - 3 # Minus 3 for '<unk>', '<s>', and '</s>'
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        for tok_span in text.lstrip().split(' '):
            ...
            if len(recats) == 2 and recats[0] in self.recategorizations and ('_' + recats[1]) in self.vocab:
                ...
                bpe_tokens.extend(self.sp_model.EncodeAsPieces(' ' + tok_span))
    ...
    def _tok_bpe(self, token, add_space=True):
        ...
        if len(recats) == 2 and recats[0] in self.recategorizations and ('_' + recats[1]) in self.vocab:
            ...
            tokk.extend(self.sp_model.EncodeAsPieces(' ' + token))
        return tokk
    ...
    def batch_encode_sentences_from_tokenized(self, tokenized, extras=None, device=torch.device('cpu')):
        ...
        if 'tokenized_ids_en' in extras[0]:
            ...
            batch_extra['input_ids_en'] = input_ids_en
            batch_extra['attention_mask_en'] = attention_mask_en
